...
#==============================================================
def save_browser_settings_with_id(location, browser_settings,browser_params):
    """
    ...
    if browser_settings is not None:
        ...
        #=================================================
        # setting is saved with prefix is browser_params["id"]
        tar_ = "browser_settings.p"
        if True:#manager_params["save_separated_profile"]:
            tar_ = browser_params["id"] + "." + tar_
            if os.path.isfile(location + tar_):
                os.remove(location + tar_)
        
            with open(location + tar_, 'wb') as f:
                cPickle.dump(browser_settings, f)
        #=================================================
def load_browser_settings_with_id(location,browser_params):
    ...
    try:
        ...
        if True:#manager_params["save_separated_profile"]:
            tar_ = browser_params["id"] + "." + tar_
            with open(location + tar_, 'rb') as f:
                browser_settings = cPickle.load(f)
        #=================================================
    except IOError:
...
def dump_profile(browser_profile_folder, manager_params, browser_params, tar_location,
    ...
    #=================================================
    # if each profile is saved with prefix is browser_params["id"]
    tar_ = "profile.tar"
    if manager_params["save_separated_profile"]:
        tar_ = browser_params["id"] + "." + "profile.tar"
        if os.path.isfile(tar_location + tar_ + '.gz'):
            tar_name = tar_ + '.gz'
        else:
            tar_name = tar_
    #=================================================
    ...
    if manager_params["save_separated_profile"]:
        save_browser_settings_with_id(tar_location, browser_settings,browser_params)
...
def load_profile(browser_profile_folder, manager_params, browser_params, tar_location, load_flash=False):
    ...
    ADDED:
        there is on extra parameter: browser_params["id"]: this is the id browser, i.e., user_name
        the profile saved must have this formation of name: id.profile.tar.gz
    """
    try:
        # Connect to logger
        logger = loggingclient(*manager_params['logger_address'])
        ...
        if manager_params["save_separated_profile"]:
            browser_settings = load_browser_settings_with_id(tar_location,browser_params)    
