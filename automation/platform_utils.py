...
def get_version():
    ...
    openwpm = "1.0.tien.1"#subprocess.check_output(["git","describe","--tags","--always"]).strip()

    ff_ini = os.path.join(os.path.dirname(__file__), '../firefox-bin/application.ini')
    #--------adding MAC specific for testing purpose ----
    # the software is intended to run on Ubuntu
    import sys
    if sys.platform == "darwin": #MAC
        ff_ini = "/Applications/Firefox.app/Contents/Resources/application.ini"
    #---------------------end------------------------------    
